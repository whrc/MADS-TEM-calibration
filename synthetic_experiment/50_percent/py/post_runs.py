...
if len(sys.argv) != 3:
    print("Usage: python post_runs.py file.yaml file.finalresults")
    sys.exit(1)
...
finalresults_file=sys.argv[2]
if finalresults_file=='':
    print(f"The filename you provided is empty")
print(f"The filename you provided is: {finalresults_file}")
fl=[finalresults_file]
...
save_file('param-'+finalresults_file+'.csv',new_list)
y=[tem.run_TEM(ig) for ig in new_list] 
y.append(tem.get_targets(1))
print(y)
save_file('out-'+finalresults_file+'.csv',y)
