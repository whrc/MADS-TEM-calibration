...
if len(sys.argv) != 3:
    print("Usage: python3 post_runs.py <path/configfilename> <path/finalresults_file>")
    sys.exit(1)
...
file_name = finalresults_file.split("/")[-1]
save_file(f'param-{file_name}.csv',new_list)
y=[tem.run_TEM(ig) for ig in new_list] 
y.append(tem.get_targets(1))
print(y)
save_file(f'out-{file_name}.csv',y)
