...
#edit timeres of soilsstructure file as needed (fsoil, line 27)
...
fsoil = "SOILSTRUCTURE_monthly_" + sc + ".nc"
...
for i in range(len(depthlist)):
	...
	datastdz[var] = np.where(datastdz['dztop']==datastdz['dzbot'],datastdz[var+'top'],(datastdz['a'] * float(dpth)) + datastdz['b'])
...
stdz = pd.concat(stdz)
#Use for monthly data
stdz = stdz.groupby(['time', 'x', 'y', 'layer']).mean().reset_index()
