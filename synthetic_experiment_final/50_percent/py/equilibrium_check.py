...
tem = get_cofig_file('config-all-md1.yaml')
CMTNUM = tem.cmtnum
...
POD='/data/workflows/sa-all-md1-r/' # path to SA sample folders
PD='/data/workflows'
path_SA='SA/'
model='results.csv'
...
for i in range(int(N)):
	print(i)
	## Read the outputs
        # run the dvmdostem with the current set of optimal parameters
...
print(POD+params)
df_param = pd.read_csv(POD+params)
df_model = pd.read_csv(POD+model,header=None)
