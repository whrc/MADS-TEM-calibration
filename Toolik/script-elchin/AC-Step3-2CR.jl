#STEP3-2
# parameters: nmax, cfall, krb
# targets: GPP[16:24], NPP[24:32], VegC(Leaf,Stem,Root)[32:48]
...
def run_TEM(x):
    ...
    return dvmdostem.get_calibration_outputs()[16:48]

def get_param_targets():
    return dvmdostem.get_calibration_outputs(calib=True)[16:48]
...
md = Mads.createproblem(initial_guess, obs, TEM_pycall;
    ...
    paramdist=["Uniform(1, 60)","Uniform(1, 60)","Uniform(1, 60)","Uniform(1, 60)",
        ...
        "Uniform(-10, -0.1)"],
    obstime=obs_time,
    paramlog=[falses(8); trues(16); falses(16)],
    problemname="Calibration_STEP3-2CR")
...
Mads.spaghettiplot(md, forward_predictions,
                       xtitle="# of observations", ytitle="GPP/NPP/VEGC",filename="STEP3-2CR_matchplot.png")
